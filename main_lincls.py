...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # init the fc layer
    init.normal_(model.fc.weight, mean=0.0, std=0.01)
    init.constant_(model.fc.bias, 0.0)
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            args.start_epoch = 0
            msg = model.set_state_dict(new_state_dict)
    ...
    parameters = list(filter(lambda p: p.trainable, model.parameters()))
    ...
    train_loader = paddle.io.DataLoader(
        ...
        # train_dataset, batch_sampler=train_sampler, num_workers=0, use_shared_memory=False)
...
def train(train_loader, model, criterion, optimizer, lr_sched, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        top1.update(float(acc1[0]), images.shape[0])
        top5.update(float(acc5[0]), images.shape[0])
...
def sanity_check(state_dict, pretrained_weights):
    ...
    for k in list(state_dict.keys()):
        ...
        assert ((state_dict[k].numpy() == state_dict_pre[k_pre]).all()), \
